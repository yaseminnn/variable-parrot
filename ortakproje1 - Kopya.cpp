...
//kurallar:
//1- deðiþken tipi gelmeden önce ya boþluk ya tab ya '(' ya '{' ya ','
//2- deðiþken tipi sonrasý ya boþluk yada tab olacak


// /*
using namespace std;
//bu fonk satiri diziye atiyor ç karakter karakter
...
int varCounter(string line, int startPos) { // line = "char a,b" =>  "a,b"
	...
	for(int i = 0; i < str.length() + 2; i++) {
		if(charArray[i] == ' ') {  // "a ,b" => ["a", " ", ",", "b"] => "a,b"
			op++;
		}
	}
	cout<<op<<endl;
	char opArray[str.length() - op + 2];
	for(int i = 0; i < str.length() + 2; i++) {
		if(charArray[i] != ' ') {  // "a ,b" => ["a", " ", ",", "b"] => "a,b"
			opArray[i] = charArray[i];
			cout<<opArray[i];
		}
	}
	cout<<endl;
	*/
	...
	// virgule kadar bi sey yapma, virgulde counter+1, noktali virgulde counter+1 ve bitir
	// char arrayOp[op.length() + 2];
	// arrayConv(op, arrayOp);
	// strcpy(arrayOp, op.c_str());
